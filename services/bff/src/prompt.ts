...
const SHAPE: Record<ArtifactType, string> = {
  Doc: `{"kind":"Doc","eyebrow":string,"title":string,"meta":string,"paragraphs":string[],"bars":[{"label":string,"value":number 0..1}]?,"barsLayout":"vertical"|"horizontal" (bar orientation)?,"callout":{"value":string (a SHORT stat like "+37%" or "31 days" — never a sentence),"label":string (short caption)}?}`,
